...
const EXTRA_LINKS = [
  { name: "Deutsche Grammatik",    href: "/grammatik/deutsch" },
  { name: "English Grammar",       href: "/grammatik/english" },
  { name: "Grammaire française",   href: "/grammatik/frances" },
  { name: "Grammatica italiana",   href: "/grammatik/italiano" },
  { name: "Gramática portuguesa",  href: "/grammatik/portugues" },
  { name: "Русская грамматика",    href: "/grammatik/ruso" },
];
  ...
  return (
    <main className="min-h-screen bg-neutral-950 p-10">
      ...
      {SECTIONS.map((section) => {
        const unlocked = isAdmin || canAccess(userNivel, section.requiredNivel);
      ...
      <section className="mb-20">
        <h2 className="text-lg mb-6 text-white/70">Más gramática</h2>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6 max-w-6xl">
          {EXTRA_LINKS.map((b) => {
            const color = COLORS[colorIndex++ % COLORS.length];
            return (
